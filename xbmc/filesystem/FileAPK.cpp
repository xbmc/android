...
bool CFileAPK::Open(const CURL& url)
{
  Close();
  ...
  {
    CLog::Log(LOGERROR, "CFileAPK::Open: Unable to open archive : '%s'",
      host.c_str());
    return false;
  }
  ...
  if (m_zip_index == -1)
  {
    ...
    CLog::Log(LOGDEBUG, "CFileAPK::Open: Unable to locate file : '%s'",
      path.c_str());
  ...
  if (rtn == -1)
  {
    CLog::Log(LOGERROR, "CFileAPK::Open: Unable to stat file : '%s'",
      path.c_str());
    zip_close(m_zip_archive);
    m_zip_archive = NULL;
    return false;
  ...
  if (!m_zip_file)
  {
    CLog::Log(LOGERROR, "CFileAPK::Open: Unable to open file : '%s'",
      path.c_str());
    zip_close(m_zip_archive);
    m_zip_archive = NULL;
    return false;
...
bool CFileAPK::Exists(const CURL& url)
{
...
int64_t CFileAPK::Seek(int64_t iFilePosition, int iWhence)
{
...
unsigned int CFileAPK::Read(void *lpBuf, int64_t uiBufSize)
{
...
int CFileAPK::Stat(struct __stat64* buffer)
{
  return Stat(m_url, buffer);
}

int CFileAPK::Stat(const CURL& url, struct __stat64* buffer)
{
  ...
  {
    CLog::Log(LOGERROR, "CFileAPK::Stat: Unable to open archive : '%s'",
      host.c_str());
    errno = ENOENT;
    return -1;
  }
...
int64_t CFileAPK::GetPosition()
{
  return m_file_pos;
}

int64_t CFileAPK::GetLength()
{
  return m_file_size;
}

int  CFileAPK::GetChunkSize()
{
  return 1;
}
